...
export const MenuModal: React.FC<Props> = ({ isOpen, setIsOpen }) => {
    ...
    return (
        <div className='w-screen h-screen flex items-center justify-center fixed top-0 left-0 z-[999] bg-white'>
            <button onClick={close}><Image src={'/icons/blackClose.svg'} width={60} height={60} alt="close" className='absolute top-[40px] left-1/2 -translate-x-1/2' /></button>
            <div className='flex flex-col space-y-10 text-center w-full'>
                {pages.map((page) => {
                    return <div key={page} className='relative'>
                        ...
                        {isActive(page) && <div className='bg-primary h-[4px] w-full absolute top-1/2 -translate-y-1/2 z-[10]' />}
                    </div>
                })}
            </div>
        </div>
